...
class Application:
    ...
    def __init__(self):
        ...
        try:
            ...
            if voices:
                ...
                print("No voices found for speech engine")
        ...
        # Add key binding for Shift key
        self.root.bind('<Shift_L>', self.add_character)
        self.root.bind('<Shift_R>', self.add_character)

        # Add binding for Delete and Backspace keys
        self.root.bind('<Delete>', lambda event: self.delete_last_char())
        self.root.bind('<BackSpace>', lambda event: self.delete_last_char())

        # Add binding for Spacebar
        self.root.bind('<space>', lambda event: self.add_space())
        
        # Add bindings for suggestion shortcuts
        self.root.bind('1', lambda event: self.action1())
        self.root.bind('2', lambda event: self.action2())
        self.root.bind('3', lambda event: self.action3())
        self.root.bind('4', lambda event: self.action4())
        ...
        # Remove speak button and add key binding for 's' key
        self.root.bind('s', lambda event: self.speak_fun())
        self.root.bind('S', lambda event: self.speak_fun())  # Also bind uppercase 'S'
    ...
    def speak_fun(self):
        try:
            if len(self.str.strip()) > 0:  # Only speak if there's text
            ...
            # Try to reinitialize the speech engine if there's an error
            try:
                self.speak_engine = pyttsx3.init()
                self.speak_engine.setProperty("rate", 100)
                voices = self.speak_engine.getProperty("voices")
                ...
                print(f"Failed to reinitialize speech engine: {str(e)}")

    def delete_last_char(self):
        if len(self.str.strip()) > 0:  # Only delete if there's text
            self.str = self.str[:-1]
            self.panel5.config(text=self.str, font=("Courier", 30), wraplength=1025)
    ...
    def add_character(self, event):
        if self.current_symbol not in ["next", "Backspace"]:
            if self.current_symbol == "  ":
                self.str = self.str + " "  # Add a single space instead of double
            else:
                self.str = self.str + self.current_symbol
            self.panel5.config(text=self.str, font=("Courier", 30), wraplength=1025)
